...
export default function SearchScanScreen() {
  ...
  /**
   * Hiển thị / cập nhật Toast mà không bị flicker.
   * Nếu Toast đang hiển thị → chỉ đổi message + reset timer.
   * Nếu chưa hiển thị → fade in bình thường.
   */
  const showToast = useCallback((message: string) => {
    ...
    toastTimerRef.current = setTimeout(() => {
      ...
      // Reset quick-add counts sau khi toast ẩn
      quickAddCountsRef.current.clear();
    }, 2500);
  }, []);

  /**
   * Thêm ngay lập tức một phần của món ăn — không navigate back,
   * không disable nút, bấm được nhiều lần.
   */
  const handleQuickAdd = useCallback(async (food: FoodItemDB) => {
    // Tăng bộ đếm cho món này
    ...
    // Thêm vào store local (optimistic)
    let mealLogId = Date.now();
    try {
      const today = new Date().toISOString().split('T')[0];
      const res = await apiClient.post(
        `/api/diary/users/${userId}/meals/${mealType}?date=${today}`,
        { foodId: food.id, quantity: 100, calories: food.calories, protein: food.protein, carb: food.carb, fat: food.fat }
      );
      if (res.data?.id) mealLogId = res.data.id;
    } catch {
      console.warn('[Offline Mode] Quick-add: Backend chưa phản hồi, dùng Local State.');
    }
    ...
    showToast(msg);
  }, [addFood, mealType, userId, showToast]);
  ...
  const handleAddCustom = async () => {
    ...
    try {
      ...
      const res = await apiClient.post(
        `/api/diary/users/${userId}/meals/${mealType}?date=${today}`,
        { foodId: 0, quantity: 100, calories: numCals, protein, carb, fat }
      );
      if (res.data?.id) mealLogId = res.data.id;
    } catch {
    ...
    addFood(mealType, {
      id: mealLogId, name: customName.trim(), calories: numCals,
      protein, carb, fat, quantity: 100,
    });
    router.back();
  };
